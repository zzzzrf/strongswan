		...
		private_XCH_SIG_t *this)
{
	...
	DBG4(DBG_IKE, "cert_payload_encoding : %B\n", &cert_payload_encoding);
...
static XCH_SIG_t *XCH_SIG_create(private_phase1_t *phase1, proposal_t *proposal)
{
	...
	else
	{
		...
		DBG1(DBG_IKE, "looking for SM peer configs matching %H...%H[%Y]\n", me, other, NULL);
		enumerator = charon->backends->create_peer_cfg_enumerator(charon->backends,
		...
		if (peer_cfg == NULL)
		{
			DBG1(DBG_IKE, "looking for SM peer configs matching %H...%H failed\n", me, other);
			free(this);
			return NULL;
		}
